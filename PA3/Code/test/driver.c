...
void fun1()
{
    ...
    // Test Case 5: Thread Exit without Mutex Release
    ...
    // Test Case 9: Thread Exit after Mutex Release
    if (current_thread->thread_id == 4)
    {
        LOG_OUT("Thread %d releasing mutex before exiting\n", current_thread->thread_id);
        mutex_release(&m);
    }
