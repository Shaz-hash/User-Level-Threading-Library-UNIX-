...
void context_switch(thread_t *prev, thread_t *next)
{
    ...
    {
        // write_formatted("Switching after n times sucessful\n");
        // write_formatted("my id is : %d \n", current_thread->thread_id);
        // write_formatted("yess5\n");
        setInterrupts(0);
    }
    return;
}
...
void thread_join(thread_t *thread)
{
    ...
    int n = thread->numberOfWaitingThreads + 1;
    // write_formatted("yes i am here %d \n", thread->numberOfWaitingThreads);
    thread->numberOfWaitingThreads = n + 1;
