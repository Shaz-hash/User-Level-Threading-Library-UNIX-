...
int main()
{
    ...
    char cwd[1024]; // creting a character array to store the current directory

    if (getcwd(cwd, sizeof(cwd)) != NULL)
    {
        LOG_OUT("Current directory: %s\n", cwd);
    }
    else
    {
        perror("getcwd() error");
    ...
    {
        fLOG_OUT(stderr, "Error reading number of elements from the input file\n");
        fclose(input_file);
        return 1;
    }
    ...
    {
        int value1, value2;
        if (fscanf(input_file, "%d %d", &value1, &value2) != 2)
        {
            fLOG_OUT(stderr, "Error reading input values from the input file\n");
            fclose(input_file);
            return 1;
        }
        arr1[i] = value1;
